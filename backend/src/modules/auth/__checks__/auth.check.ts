...
import { signIn, userFromToken, AuthError, type Session } from '../auth.service';
import { borrarUsuariosDePrueba, borrarYAnotar, crearFirmaConSesion, clavePrueba, informarLimpieza } from './helpers';
...
(async () => {
  /*
   * LA LIMPIEZA VA EN UN `finally`. Antes corria al final del cuerpo, asi que
   * cualquier excepcion a mitad del check saltaba el borrado y dejaba firmas y
   * cuentas en la base — que en local es la de PRODUCCION. Lo creado se declara
   * fuera del `try` para que el `finally` alcance lo que de verdad llego a
   * existir.
   */
  let sa: Session | undefined;
  let sb: Session | undefined;
  try {
    sa = await crearFirmaConSesion(A);
    sb = await crearFirmaConSesion(B);
    ...
    // Limpieza.
    const limpieza: string[] = await borrarUsuariosDePrueba(supabase!, marca);
    const firmas = [sa?.user.firmId, sb?.user.firmId].filter((id): id is string => typeof id === 'string');
    if (firmas.length > 0) await borrarYAnotar(limpieza, 'firms', supabase!.from('firms').delete().in('firm_id', firmas));
    await borrarYAnotar(limpieza, 'firms (nit 902)', supabase!.from('firms').delete().eq('nit', `902${marca}`));
