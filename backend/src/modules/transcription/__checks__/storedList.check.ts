...
(async () => {
   ...
   * excepcion a mitad del check la saltaba y dejaba firmas, cuentas y
   * transcritos en la base — que en local es la de PRODUCCION. Cada firma se
   * anota aqui en cuanto existe, para que el `finally` borre lo que de verdad
   * se creo.
   */
  ...
  try {
    ...
    if (ids.length > 0) await borrarYAnotar(limpieza, 'transcriptions', c.from('transcriptions').delete().in('firm_id', ids));
