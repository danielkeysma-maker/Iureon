...
import { OpenRouterMultiEngineService, AgentExecutionStep } from '../services/openrouter.service.js';

const aiService = new OpenRouterMultiEngineService();

/**
 * Controller para streaming SSE de los pasos del agente RAG Multi-Motor
 */
  ...
  try {
    ...
    const result = await aiService.executeMultiEnginePipeline(
      ...
      (step: AgentExecutionStep) => {
        // Enviar log en vivo al cliente por SSE
        sendEvent('AGENT_LOG', step);
      }
    );

    // Enviar resultado final con el borrador generado por Claude Opus 5
