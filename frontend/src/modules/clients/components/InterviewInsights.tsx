    ...
    <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="font-bold text-slate-900 text-xs">Jurisprudencia relacionada</h4>
        ...
        <button
          ...
          disabled={cargando}
          className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 rounded-lg text-[11px] font-semibold flex items-center gap-1.5 shrink-0 disabled:opacity-60"
        >
      ...
      {reason && <p className="text-[11px] text-slate-600 bg-slate-50 border border-slate-200 rounded-lg p-2">{reason}</p>}
      ...
      {suggestions.length > 0 && (
        <div className="space-y-2">
          {suggestions.map((s, i) => (
            <div key={`${s.providencia}-${i}`} className="border border-slate-200 rounded-lg p-3 space-y-1.5">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="text-[10px] font-mono text-slate-400 shrink-0" title="Cercanía semántica, no aplicabilidad">
                  {(s.similarity * 100).toFixed(0)}%
                </span>
              </div>

              <p className="text-[11px] text-slate-700 leading-relaxed">{s.excerpt}</p>

              {/*
                What the client said that produced this. Without it the card is
                an assertion; with it the lawyer can dismiss it in a second.
              */}
              <p className="text-[10px] text-slate-500 border-l-2 border-slate-200 pl-2">
              ...
              {s.sourceUrl && (
                <a
                  ...
                  className="text-[10px] font-semibold text-blue-900 hover:underline flex items-center gap-1"
                >
                  Ver la providencia
