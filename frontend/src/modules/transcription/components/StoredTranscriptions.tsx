  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl">
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
        <div>
          <h4 className="font-bold text-slate-900 text-xs">Transcripciones guardadas</h4>
        ...
        <button
          onClick={onRefresh}
          className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 rounded-lg text-[11px] font-semibold flex items-center gap-1.5"
        >
      ...
      {isLoading && items.length === 0 ? (
        <p className="px-4 py-4 text-[11px] text-slate-500">Cargando…</p>
      ) : items.length === 0 ? (
        ...
        <div className="divide-y divide-slate-100 max-h-72 overflow-y-auto">
          {items.map((item) => (
            <div key={item.id} className="px-4 py-2.5 flex items-center gap-3">
              ...
              >
                <p className="text-[11px] font-semibold text-slate-800 truncate group-hover:text-blue-900">
              ...
              */}
              {confirmando === item.id ? (
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-[10px] text-rose-800">¿Borrar?</span>
                  <button
                    onClick={() => {
                      onDelete(item.id);
                      setConfirmando(null);
